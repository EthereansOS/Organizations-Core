...
contract MVDProxy is IMVDProxy {
    ...
    constructor(
        ...
        init(
            ...
            stateHolderAddress,
            functionalityModelsManagerAddress,
    ...
    function init(
        ...
        IMVDProxyDelegate(_delegates[1] = functionalityProposalManagerAddress)
            .setProxy();
        ...
        IMVDProxyDelegate(_delegates[4] = functionalitiesManagerAddress)
            .setProxy();
    ...
    function flushToWallet(
        ...
        require(
            IMVDFunctionalitiesManager(_delegates[4]).isAuthorizedFunctionality(
                msg.sender
            ),
            "Unauthorized action!"
        );
        ...
        returns (address oldAddress)
    {
        ...
        if (position == 5) {
            IMVDWallet(_delegates[5]).setNewWallet(
                payable(newAddress),
                _delegates[0]
            );
        }
    ...
    function changeProxy(address newAddress, bytes memory initPayload)
        public
        override
    {
        require(
    ...
    {
        return
            IMVDFunctionalityProposalManager(_delegates[1]).isValidProposal(
                proposal
            );
    }
    ...
    function newProposal(
    ...
    ) public override returns (address proposalAddress) {
        emergencyBehavior(emergency);

        IMVDFunctionalityModelsManager(_delegates[3])
            .checkWellKnownFunctionalities(
            codeName,
            submittable,
            methodSignature,
            ...
            IMVDFunctionalitiesManager functionalitiesManager
         = IMVDFunctionalitiesManager(_delegates[4]);

        IMVDFunctionalityProposal proposal = IMVDFunctionalityProposal(
            proposalAddress = IMVDFunctionalityProposalManager(_delegates[1])
                .newProposal(
                codeName,
                location,
                methodSignature,
        ...
        proposal.setCollateralData(
            ...
            needsSender,
            msg.sender,
            functionalitiesManager.hasFunctionality("getVotesHardCap")
                ? toUint256(read("getVotesHardCap", ""))
                : 0
        );
        ...
        if (
            !IMVDFunctionalitiesManager(_delegates[4]).hasFunctionality(
                "startProposal"
            ) ||
            !IMVDFunctionalitiesManager(_delegates[4]).hasFunctionality(
                "disableProposal"
    ...
    function emergencyBehavior(bool emergency) private {
        ...
        (address loc, , string memory meth, , ) = IMVDFunctionalitiesManager(
            _delegates[4]
        )
            .getFunctionalityData("getEmergencySurveyStaking");
        (, bytes memory payload) = loc.staticcall(
            abi.encodeWithSignature(meth)
        );
        uint256 staking = toUint256(payload);
        if (staking > 0) {
            IERC20(_delegates[0]).transferFrom(
                msg.sender,
                address(this),
                staking
            );
    ...
    function startProposal(address proposalAddress) public override {
        ...
        (address location, , , , ) = IMVDFunctionalitiesManager(_delegates[4])
            .getFunctionalityData("startProposal");
        require(
            location == msg.sender,
            "Only startProposal Functionality can enable a delayed proposal"
        );
        require(
            IMVDFunctionalityProposalManager(_delegates[1]).isValidProposal(
                proposalAddress
            ),
            "Invalid Proposal Address!"
        );
    ...
    function disableProposal(address proposalAddress) public override {
        ...
        (address location, , , , ) = IMVDFunctionalitiesManager(_delegates[4])
            .getFunctionalityData("disableProposal");
        require(
            location == msg.sender,
            "Only disableProposal Functionality can disable a delayed proposal"
        );
    ...
    function transfer(
        ...
        require(
            ...
            "Only functionalities can transfer Proxy balances!"
        );
    ...
    function transfer721(
        ...
        IMVDWallet(_delegates[5]).transfer(
            receiver,
            tokenId,
            data,
            safe,
    ...
    function setProposal() public override {
        IMVDFunctionalityProposalManager(_delegates[1]).checkProposal(
            msg.sender
        );
        ...
        ) = functionalitiesManager.getFunctionalityData("checkSurveyResult");

        (bool surveyResult, bytes memory response) = addressToCall.staticcall(
            abi.encodeWithSignature(methodSignature, msg.sender)
        );

        surveyResult = toUint256(response) > 0;

        bool collateralCallResult = true;
        (addressToCall, , methodSignature, , ) = functionalitiesManager
            .getFunctionalityData("proposalEnd");
        if (addressToCall != address(0)) {
            functionalitiesManager.setCallingContext(addressToCall);
            (collateralCallResult, ) = addressToCall.call(
        ...
        if (proposal.isEmergency()) {
            (addressToCall, , methodSignature, , ) = functionalitiesManager
                .getFunctionalityData("getEmergencySurveyStaking");
            (, response) = addressToCall.staticcall(
        ...
        if (collateralCallResult) {
            try functionalitiesManager.setupFunctionality(msg.sender) returns (
                bool managerResult
            ) {
        ...
        if (collateralCallResult) {
            ...
            if (staking > 0) {
                IERC20(tokenAddress).transfer(
                    surveyResult ? proposal.getProposer() : walletAddress,
                    staking
                );
            }
        }
    ...
    function read(string memory codeName, bytes memory data)
        public
        override
        view
        returns (bytes memory returnData)
    {
        (address location, bytes memory payload) = IMVDFunctionalitiesManager(
            _delegates[4]
        )
            .preConditionCheck(codeName, data, 0, msg.sender, 0);
    ...
    function submit(string memory codeName, bytes memory data)
        public
        override
        payable
        returns (bytes memory returnData)
    {
        ...
        IMVDFunctionalitiesManager manager = IMVDFunctionalitiesManager(
            _delegates[4]
        );
        (address location, bytes memory payload) = manager.preConditionCheck(
            codeName,
            data,
            1,
            msg.sender,
    ...
    function callFromManager(address location, bytes memory payload)
        public
        override
        returns (bool, bytes memory)
    {
        require(
            msg.sender == _delegates[4],
            "Only Functionalities Manager can call this!"
        );
    ...
    function emitFromManager(
        ...
        emit FunctionalitySet(
            codeName,
            proposal,
            replaced,
            replacedSourceLocation,
            replacedSourceLocationId,
            location,
            submittable,
            methodSignature,
            isInternal,
    ...
    function emitEvent(
        ...
        bytes memory firstIndex,
        bytes memory secondIndex,
        bytes memory data
    ) public override {
        require(
            IMVDFunctionalitiesManager(_delegates[4]).isAuthorizedFunctionality(
            ...
            "Only authorized functionalities can emit events!"
        );
        emit Event(
            eventSignature,
            keccak256(firstIndex),
            keccak256(secondIndex),
            data
        );
    }
