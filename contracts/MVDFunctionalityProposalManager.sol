...
contract MVDFunctionalityProposalManager is IMVDFunctionalityProposalManager {
    ...
    modifier onlyProxy() {
        require(
            msg.sender == address(_proxy),
            "Only Proxy can call this functionality"
        );
        _;
    }
    ...
    ) public override onlyProxy returns (address) {
        return
            setProposal(
                codeName,
                location,
    ...
    function preconditionCheck(
        ...
        require(
            (hasCodeName || (!hasCodeName && !hasReplaces))
                ? location != address(0)
                : true,
            "Cannot have zero address for functionality to set or one time functionality to call"
        );

        require(
            location == address(0) || !compareStrings(methodSignature, ""),
            "Cannot have empty string for methodSignature"
        );

        require(
            hasCodeName || hasReplaces
                ? true
                : compareStrings(methodSignature, "callOneTime(address)"),
            "One Time Functionality method signature allowed is callOneTime(address)"
        );
        ...
        require(
            hasCodeName && functionalitiesManager.hasFunctionality(codeName)
                ? compareStrings(codeName, replaces)
                : true,
            "codeName is already used by another functionality"
        );

        require(
            hasReplaces
                ? functionalitiesManager.hasFunctionality(replaces)
                : true,
            "Cannot replace unexisting or inactive functionality"
        );
    }
