...
contract VotingToken is IERC20, IVotingToken {
    ...
    function transferFrom(
        ...
        if (
            ...
            !(IMVDFunctionalityProposalManager(
                IMVDProxy(_proxy).getMVDFunctionalityProposalManagerAddress()
            )
            ...
            _approve(
                sender,
                txSender,
                _allowances[sender][txSender] = sub(
                    _allowances[sender][txSender],
                    amount,
    ...
    function burn(uint256 amount) public override {
        _balances[msg.sender] = sub(
            _balances[msg.sender],
            amount,
            "VotingToken: burn amount exceeds balance"
        );
        _totalSupply = sub(
            _totalSupply,
            amount,
            "VotingToken: burn amount exceeds total supply"
        );
